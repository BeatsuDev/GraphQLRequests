...
class QueryBuilder(abc.ABC):
    ...
    def _process_options(self, options: dict) -> None:
        ...
        if options:
            raise ValueError("The following options are not valid options for a" \
    ...
    def _generate_fields(self) -> str:
        ...
        for field, field_type in fields_to_generate.items():
            ...
            #   name
            # }
            elif self._resolved_fields[field] in self.SUPPORTED_TYPES or (type(field_type) == enum.EnumType):
